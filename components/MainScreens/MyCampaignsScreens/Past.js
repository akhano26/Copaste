import { StyleSheet, Text, View,FlatList,TouchableOpacity,Image,useNavigation} from 'react-native'
import React from 'react'
import { responsiveAverage,responsiveHeight,responsiveWidth,dp } from '../../../assets/utility'

const Past = ({storesData}) => {
  const renderItem = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity key={index} style={styles.single_post_con}>
        ...
        <Text style={{ fontSize:dp(14),color: 'black', lineHeight: responsiveHeight(3.5), fontFamily: 'Poppins-Bold' }} >{item.storeName}</Text>
       
        <Text style={{ color: 'black', lineHeight: responsiveHeight(3.5), fontFamily: 'Poppins-Bold',fontSize:dp(14) }}>{item.price}</Text>
        {item.paidStatus?
         <Text style={{ fontSize:dp(12), color: '#6061F6', lineHeight: responsiveHeight(3.5),fontSize:dp(12), fontFamily: 'Poppins-Regular' }}>Paid</Text>:
         <Text style={{ fontSize:dp(12),color: '#B00020', lineHeight: responsiveHeight(3.5),fontSize:dp(12), fontFamily: 'Poppins-Regular' }}>Rejected Admin</Text>
         }
      </TouchableOpacity>
    );
  }
