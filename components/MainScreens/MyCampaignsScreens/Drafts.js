...
const Drafts = ({storesData}) => {
const navigation=useNavigation()
  const renderItem = ({ item, index }) => {
  
    return (
      <TouchableOpacity onPress={()=>{navigation.navigate('DraftScreen1',{itemData:item})
      
      }} key={index} style={styles.single_post_con}>
        ...
        {item.aproveStatus?
         <Text style={{ color: '#6061F6', lineHeight: responsiveHeight(3.5),fontSize:dp(12), fontFamily: 'Poppins-Regular' }}>Approved</Text>:
         <Text style={{ color: '#B00020', lineHeight: responsiveHeight(3.5),fontSize:dp(12), fontFamily: 'Poppins-Regular' }}>Pending</Text>
         }
      </TouchableOpacity>
    );
  }
